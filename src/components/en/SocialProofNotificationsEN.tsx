...
const notifications = [
  { name: "Sarah", location: "Los Angeles", source: "Instagram" },
  { name: "Emily", location: "New York", source: "Facebook" },
...
export const SocialProofNotificationsEN = () => {
      ...
      <div className="fixed bottom-20 md:bottom-6 left-2 md:left-4 z-40 animate-slide-up">
        <div className="bg-highlight text-highlight-foreground rounded-xl shadow-medium py-2.5 px-3 flex items-center gap-2.5 max-w-[260px]">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold flex items-center gap-1">Hurry!</p>
            <p className="text-[11px] font-medium">Only <span className="font-black">{spotsLeft}</span> spots left at this price</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed bottom-20 md:bottom-6 left-2 md:left-4 z-40 animate-slide-up">
      <div className="bg-white rounded-xl shadow-medium border border-border py-2.5 px-3 flex items-center gap-2.5 max-w-[260px]">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-foreground truncate">{notification.name} from {notification.location}</p>
          <p className="text-[11px] text-muted-foreground flex items-center gap-1">
            <span>{sourceIcons[notification.source]}</span>
            just bought the Cone Pack 💖
          </p>
        </div>
      </div>
    </div>
